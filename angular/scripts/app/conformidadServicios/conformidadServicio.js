...
(function () {
    ...
    ConformidadServicioCtrl.$inject = ['$scope', '_', 'RESTService', 'AlertFactory'];

    function ConformidadServicioCtrl($scope, _, RESTService, AlertFactory)
    {
        ...
        $.fn.modal.Constructor.prototype.enforceFocus = function () {};
         function getDataForProforma () {
            RESTService.all('conformidadServicios/data_formProf', '', function(response) {
        ...
        function getDataOrdenCompra() {
            console.log("djdjjdjdjd");
            RESTService.all('conformidadServicios/data_formOrdenCompra', '', function(response) {
                if (!_.isUndefined(response.status) && response.status) {

                    ordenes_completas=response.dataOrdenesConformidad;
                        // console.log("aaaaaaaaaaaa");
                    console.log(ordenes_completas);
                    console.log("aaaaaaaaaaaa");
        ...
        cCodConsecutivoOS.change(function () {
                ...
                if(cCodConsecutivoOS.val()!=''){
                    ...
                    RESTService.get('conformidadServicios/getDetalle_ordenCompra', id, function(response) {
                     if (!_.isUndefined(response.status) && response.status) {
                          var data=response.valConf;
                          var cont=0;
                          if(idMovimiento.val()==''){
         ...
         btn_imprimirMovimiento.click(function(e){
            ...
            if(id!=''){
              ...
              $scope.loadMovimientoEntregaPDF('conformidadServicios/pdfMovemen', data);
            }
        });
        ...
        {   
            titlemodalMovimieto.html('Editar Conformidad');
          
            RESTService.get('conformidadServicios/findMovement', id, function(response) {
                ...
                if (!_.isUndefined(response.status) && response.status) {
                   ...
                   console.log(mov_ar);
                    ...
                    titlemodalMovimieto.html('Editar Conformidad '+'['+ data_p.idMovimiento+ ']');
        ...
        function getlotes(){
            ...
            if(id!=''){
                 RESTService.get('conformidadServicios/validateLoteMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
                      if(response.data=="N"){
         ...
         $('#ProcesarTransferenciaBoton').click(function(e){
          ...
          if(articulo_mov_det.html()!=""){
                    RESTService.get('conformidadServicios/validaDetalleMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
         ...
         $scope.ProcesarTransferencia = function(){
            ...
            RESTService.get('conformidadServicios/procesarTransferenciaMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
        ...
        function getkitDet(codigo){
             RESTService.get('conformidadServicios/getKitMovement', codigo, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
                      var data_p = response.data;
        ...
        function getLocaStock(idl,ident,idPrAl,idLocalizacion){
            var idLocali=$("#"+ident);
            var id=idl;
             RESTService.get('conformidadServicios/getLocaStockMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
        ...
        $scope.guardarMovimientoDetalle = function(){
            ...
            if(bval){
                ...
                RESTService.updated('conformidadServicios/saveEntrega', movimiento_id, paramsCabezera, function(response) {
                    if (!_.isUndefined(response.status) && response.status) {
                        titlemodalMovimieto.html('Nueva Conformidad'+'['+ response.code+ ']');
                        AlertFactory.textType({
                            title: '',
        ...
        function addArticuloTable(idDetalle,idProducto,desProducto,cantProducto,ver,codigo,tipo,codl,datl,idAlmacen,idLocalizacion,costo,costo_total,precio,presio_total,cantidaPD){
            ...
            $('.verUpdate').click(function(e){
                ...
                console.log("ver codigo ",codeShow);
             ...
             $('.delMovPro').click(function (e) {
                ...
                AlertFactory.confirm({
                    ...
                    if(idMovimiento.val()!='' && idDetalle!=0){
                        ...
                        RESTService.get('conformidadServicios/deleteDetalleST', id, function(response) {
                        if (!_.isUndefined(response.status) && response.status) {
                                   AlertFactory.textType({
                                    title: '',
                     ...
                     if (!_.isUndefined(response.status) && response.status) {
                        ...
                        if(identSerAr.val()!=""){
                            ...
                            for (var i = 0; i < cant ; i++) {
                             ...
                             $("#btn_ver"+identSerAr.val()).attr('data-tShow',"SN");

                              $("#id_check"+identSerAr.val()).prop("checked", true);
        ...
        //         if (bval) {
        //              camposunicos = camposunicos.join(',');
        //              RESTService.get('conformidadServicios/valida_series_serve', camposunicos, function(response) {
        //              if (!_.isUndefined(response.status) && response.status) {
        //                 if(cant==cont_table){
        //                 if(identSerAr.val()!=""){
        //                     var updteSN = aartMSN.filter(function(car) {
         ...
         $scope.addLoteExi = function(){
            ...
            if (bval) {
                ...
                // addArticuloTable(idProductoMll.val(),desProductoMll.val(),cantProductoMll.val(),ver,codigoLr,tipoArtLr,codl,datl,idAlmacen,idLocalizacion,costo,costo_total,precio,precioTotal);
                 ...
                 $("#idLEn"+codigo_actual).val(idLoteMll.val());
                 $("#btn_ver"+codigo_actual).attr('data-lote',codigoLoteMll.val());
                $("#id_check"+codigo_actual).prop("checked", true);
                    $('.i-checks').iCheck({
                    checkboxClass: 'icheckbox_square-green'
        ...
        $scope.EliminarMovimiento = function(){
            ...
            RESTService.get('conformidadServicios/deleteMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
                    var dta=response.data;
                    if(dta[0]['Mensaje']!=""){
                         AlertFactory.textType({
        ...
        $scope.addSeleccSerie = function(){
            ...
            if (bval) {
                ...
                RESTService.get('conformidadServicios/validateCantSerieMovement', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
                    if(response.data=='N'){
                       AlertFactory.textType({
                        title: '',
        ...
        $scope.addArtLote = function(){
            console.log(codigo_actual,"este es el codigo actual");
            var bval = true;
            ...
            console.log(idLoteML2.val(),"idLote idLoteML2");
            if (bval) {
                if(idLoteML2.val()==""){
        ...
        //         RESTService.updated('conformidadServicios/saveMovimientoMovement', movimiento_id, params, function(response) {
        //             if (!_.isUndefined(response.status) && response.status) {
        //                 titlemodalMovimieto.html('Nueva Conformidad '+'['+ response.code+ ']');
        //                 AlertFactory.textType({
        //                     title: '',
        ...
        {
            titlemodalMovimieto.html('Nueva Conformidad');
            modalMovimieto.modal('show');
            // var verProforma='CR';
            cargar_ordenCompra();
            // cargar_notas()
        ...
        //     RESTService.get('conformidadServicios/get_venta_detalle', idventa, function (response) {
        //         if (!_.isUndefined(response.status) && response.status) {
        ...
        //     $.post("conformidadServicios/get_ventas_entrega", {},
        ...
        function getDataFormMovement () { 
            RESTService.all('conformidadServicios/data_formRegi', '', function(response) {
                if (!_.isUndefined(response.status) && response.status) {
        ...
        table_container_Register_Movement.jtable({
            ...
            toolbar: {
                items: [{
                    ...
                    click: function () {
                        $scope.openDoc('conformidadServicios/excel', {});
                    }
                },{
                    cssClass: 'btn-danger-admin',
                    text: '<i class="fa fa-plus"></i> Nueva Conformidad',
                    click: function () {
                        newMovimiento();
                    }
                }]
            ...
            fields: {
                ...
                idTipoOperacion: {
                    ...
                    options: base_url + '/conformidadServicios/getAllOperation' 
                },
                idUsuario: {
                    title: 'Usuario',
                    options: base_url + '/conformidadServicios/getAllUsers' 
                },
                estado: {
                    title: 'Estado',
        ...
        table_container_cc2.jtable({
            ...
            sorting: true,
             cache: false,
            actions: {
                listAction: base_url + '/conformidadServicios/getArticulosSelect'
            },
            toolbar: {
                items: [{
                    cssClass: 'buscador',
        ...
        table_container_cc2.jtable({
            ...
            sorting: true,
             cache: false,
            actions: {
                listAction: base_url + '/conformidadServicios/getArticulosMinKit'
            },
            toolbar: {
                items: [{
                    cssClass: 'buscador',
            ...
            table_container_cc4.jtable({
                ...
                actions: {
                    ...
                    return $.Deferred(function ($dfd) {
                        $.ajax({
                            url:  base_url + '/conformidadServicios/'+url,
                            type: 'POST',
                            dataType: 'json',
