...
class ProformaViewRepository implements ProformaViewInterface
{
    protected $model;
 private static $_ACTIVE = 'A';
    public function __construct(ProformaView $model)
    {
        $this->model = $model; 
    ...
    public function destroy_proforma_servicio($id,$no,$mant)
    {
         ...
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_EliminaProformaViewMO '$id','$no','$mant'");
         return $destroy;
    }
    public function destroy_proforma_repuesto($id,$no,$mant)
    {
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_EliminaProformaViewDetalle '$id','$no','$mant'");
         return $destroy;
    }
     public function cambiar_estado($id,$no,$estado,$usuario)
    {
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_ActualizaEstadoProformaView '$id','$no','$estado','$usuario'");
         return $destroy;
    }
     public function find_proforma($conse,$nro)
    {
        $mostrar3=DB::select("SELECT prof.nIdDscto as nIdDsctoPr, des.nPorcDescuento as porDes , des.nMonto as montoDes, prof.nEstimadoHoras as nEstimadoHoras,prof.idAsesor as idAsesorProformaView, prof.cCodConsecutivo as cCodConsecutivo,prof.nConsecutivo as nConsecutivo,prof.dFechaRegistro as dFechaRegistro,prof.nTotalMO as nTotalMO,prof.nTotalDetalle as nTotalDetalle,prof.nSubTotal as nSubTotal,prof.nTotal as nTotal,prof.nImpuesto as nImpuesto,prof.iEstado as iEstado,os.iEstado as est ,os.idCliente as idCliente,os.cPlacaVeh as cPlacaVeh, os.cMotor as cMotor,os.nKilometraje as nKilometraje, os.cColor as cColor,  cl.id_tipocli as idTipoCliente,cl.documento as documento,cl.razonsocial_cliente as razonsocial_cliente, os.cCodConsecutivo as cCodConsecutivoOS, os.nConsecutivo as nConsecutivoOS, os.IdMoneda as IdMoneda,mo.Descripcion as moneda,os.idcCondicionPago as idcCondicionPago,cp.description as condicionPago,os.idAsesor as idAsesor,ase.descripcion as asesor FROM ERP_OrdenServicio as os inner join ERP_Moneda as mo on os.IdMoneda=mo.IdMoneda inner join ERP_CondicionPago as cp on cp.id=os.idcCondicionPago INNER JOIN ERP_Clientes as cl on cl.id=os.idCliente INNER JOIN ERP_TipoCliente as tc on tc.id=cl.id_tipocli inner join ERP_ProformaView as prof on prof.cCodConsecutivoOS=os.cCodConsecutivo  inner join ERP_Asesores as ase on ase.id=os.idAsesor  left join ERP_Descuentos as des on des.id=prof.nIdDscto  where prof.nConsecutivoOS=os.nConsecutivo and prof.cCodConsecutivo='$conse' AND prof.nConsecutivo='$nro'");
          return $mostrar3;
    }
   
    public function find_proforma_repuestos($conse,$nro)
    { 
      $mostrar3=DB::select("select des.nPorcDescuento as porDes, des.nMonto as montoDes, od.id_tipototal as totaltipo,tit.descripcion as descripcioText, pr.id as idProducto, od.id as idDetalleRepues,* from ERP_ProformaViewDetalle as od inner join ERP_Productos as pr on pr.id=od.idProducto inner join ERP_TipoTotalOS as tit on tit.id=od.id_tipototal left join ERP_Descuentos as des on des.id=od.nIdDscto where od.cCodConsecutivo='$conse' and od.nConsecutivo='$nro'");
      return $mostrar3;

    } 
    public function find_proforma_servicios($conse,$nro)
    { 
      $mostrar3=DB::select("select des.nPorcDescuento as porDes, des.nMonto as montoDes,od.id_tipototal as totaltipo,tit.descripcion as descripcioText, pr.id as idProducto, od.id as idDetalleServicio,* from ERP_ProformaViewMO as od inner join ERP_Productos as pr on pr.id=od.idProducto inner join ERP_TipoTotalOS as tit on tit.id=od.id_tipototal left join ERP_Descuentos as des on des.id=od.nIdDscto  where od.cCodConsecutivo='$conse' and od.nConsecutivo='$nro'");
      return $mostrar3;
    ...
    public function actualizar_proforma(
                ...
                $usuario){
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_ActualizaProformaView 
                '$cCod',
                '$nCons',
                '$cCodOS',
                '$nConsOS',
    ...
    public function actualizar_ProformaView_detalle(
         ...
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_ActualizaProformaViewDetalle 
                '$cCod','$ncon','$cont','$id_repuesto_array','$id_repuesto_cantidad','$id_repuesto_precio','$total','$id_repuesto_impuesto','$id_repuesto_tipoTotal','$montoRepu','$porRepu','$idDescuenRepues','$staOperacionRepu','$operacionGraRep','$modo_array_repuesto','$usuario'");
         // return $destroy;
    }
   public function actualizar_ProformaView_MO($cCodConsecutivo,$res,$idDetalleGrup,$id_revision_array,$totald,$impuesto_servicio,$id_tipo_array,$montoDeta,$porDeta,$cantidDeta,$precio_array,$idDescuenDeta,$staOperacion,$totalO,$modo_array_serv,$usuario){
         $pdo=DB::connection()->getPdo();
         $destroy=DB::update("SET NOCOUNT ON; EXEC ST_ActualizaProformaViewMO 
     ...
     public function destroy_ProformaView_detalleSer($id,$no,$deta)
    {
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_EliminaProformaViewMO '$id','$no','$deta'");
         return $destroy;
    }
     public function destroy_ProformaView($id,$no)
    {
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_EliminaProformaView '$id','$no'");
         return $destroy;
    }
     public function destroy_ProformaView_detalleRepuesto($id,$no,$deta)
    {
         $pdo=DB::connection()->getPdo();
         $destroy=DB::select("SET NOCOUNT ON; EXEC ST_EliminaProformaViewDetalle '$id','$no','$deta'");
         return $destroy;
    }
    ...
    public function get_proformas_entrega(){
          ...
          $mostrar3=DB::select("  
                SELECT os.iEstado as est ,os.idCliente as idCliente,os.cPlacaVeh as cPlacaVeh,  cl.id_tipocli as idTipoCliente,cl.documento as documento,cl.razonsocial_cliente as razonsocial_cliente, os.cCodConsecutivo as cCodConsecutivo, os.nConsecutivo as nConsecutivo, os.IdMoneda as IdMoneda,mo.Descripcion as moneda,os.idAsesor as idAsesor FROM ERP_ProformaView as os inner join ERP_Moneda as mo on os.IdMoneda=mo.IdMoneda  INNER JOIN ERP_Clientes as cl on cl.id=os.idCliente where os.cfacturado ='N'");
          return $mostrar3;
    ...
    public function get_proformas_devolucion(){
          ...
          $mostrar3=DB::select("  
                SELECT os.iEstado as est ,os.idCliente as idCliente,os.cPlacaVeh as cPlacaVeh,  cl.id_tipocli as idTipoCliente,cl.documento as documento,cl.razonsocial_cliente as razonsocial_cliente, os.cCodConsecutivo as cCodConsecutivo, os.nConsecutivo as nConsecutivo, os.IdMoneda as IdMoneda,mo.Descripcion as moneda,os.idAsesor as idAsesor FROM ERP_ProformaView as os inner join ERP_Moneda as mo on os.IdMoneda=mo.IdMoneda  INNER JOIN ERP_Clientes as cl on cl.id=os.idCliente where os.cfacturado ='N' and (os.iEstado='2' or os.iEstado='3')");
          return $mostrar3;
    }
     public function get_detalle_entrada($conse,$nro){ 
   
          $mostrar3=DB::select("select pr.costo as costo2,pr.costo as costo_total,pr.id as idProducto, od.id as idDetalleRepues,* from ERP_ProformaViewDetalle as od inner join ERP_Productos as pr on pr.id=od.idProducto  where   pr.type_id ='1' and od.nCantidadPendienteEntregar >0 and od.cCodConsecutivo='$conse' and od.nConsecutivo='$nro'
");
          return $mostrar3;
    }
     public function get_detalle_entrada_Devolucion($conse,$nro){ 
   
          $mostrar3=DB::select("select pr.costo as costo2,pr.costo as costo_total,pr.id as idProducto, od.id as idDetalleRepues,* from ERP_ProformaViewDetalle as od inner join ERP_Productos as pr on pr.id=od.idProducto  where   pr.type_id ='1'  and od.cCodConsecutivo='$conse' and od.nConsecutivo='$nro'
");
          return $mostrar3;
    }
