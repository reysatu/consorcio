...
class CajaDiariaRepository implements CajaDiariaInterface
{
 ...
 private static $_ACTIVE = 'A';
    ...
    public function get_ventaComproTipoPago($date,$usuario){
        $mostrar=DB::select("select SUM(vp.monto_pago) as total,cp.id as idCondicionPago,cp.description as condicionPago from ERP_VentaFormaPago as vp inner join erp_venta as v on vp.idventa=v.idventa inner join ERP_Clientes as cl on v.idcliente=cl.id inner join ERP_FormasPago as fp on fp.codigo_formapago=vp.codigo_formapago inner join ERP_CondicionPago as cp on cp.id=v.condicion_pago where convert(date,v.fecha_emision)='$date' and v.idcajero='$usuario' GROUP BY v.condicion_pago ,cp.id,cp.description");
        return $mostrar; 
    }
