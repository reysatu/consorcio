...
namespace App\Http\Recopro\ViewSerie;
use Illuminate\Support\Facades\DB;

class ViewSerieRepository implements ViewSerieInterface
{
    protected $model;

    public function __construct(ViewSerie $model)
    {
        $this->model = $model; 
    }
    ...
    {
        return $this->model->where('nombreViewSerie', $code)->first();
    }

    public function find($id)
    {
        $mostra=DB::select("SELECT pr.id as idArticulo,pr.description as Articulo,Se.idViewSerie as idViewSerie,Se.nombreViewSerie AS nombreViewSerie,Se.cPlacaVeh as cPlacaVeh, Se.chasis as chasis,Se.idTipoCompraVenta as idTipoCompraVenta,Se.nPoliza as nPoliza, Se.nLoteCompra as nLoteCompra, Se.motor as motor, Se.anio_fabricacion as anio_fabricacion, Se.anio_modelo as anio_modelo, Se.color as color FROM ERP_ViewSerie as Se inner join ERP_Productos as pr on Se.idArticulo=pr.id where Se.idViewSerie=$id");
        return $mostra;
    }
    ...
    public function getViewSeries($idArticulo){
        $mostra=DB::select("select idArticulo from ERP_ViewSerie where idArticulo=$idArticulo");
        return $mostra;
    }
