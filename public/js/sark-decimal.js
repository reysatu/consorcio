...
function generateSarkDecimal() {
    ...
    $( document ).on(  "keydown", "input[data-behaviour=decimal]", function(e){
        ...
        var mxDecimal = typeof curElem.attr( "maxdecimal" ) == "undefined" ? fieldDecimals : curElem.attr( "maxdecimal" );
        if( strSlct == endSlct ){
            if( !( ( keycode < 48 || keycode > 57 )  && ( keycode < 96 || keycode > 105 ) ) ){
                if( ( fraction[0].length <= strSlct || mxLength <= curElem.val().length ) && fraction[0].length != strSlct && fraction[1].length <= mxDecimal && strSlct != curElem.val().length  ){
                    curElem.val( sark_behav.replaceStr( strSlct, strSlct, inpVal, curElem.val() ) );
                }
                if( fraction[0].length <= strSlct && fraction[1].length < mxDecimal ){
